...
# Remove old per-node Proxmox dev entries for ALSA only. They are brittle because
# USB card numbers can change between boots. Preserve every non-/dev/snd device.
TMP=$(mktemp)
awk '!($1 ~ /^dev[0-9]+:/ && $0 ~ /path=\/dev\/snd\//)' "$CONF" > "$TMP"
cat "$TMP" > "$CONF"
rm -f "$TMP"

grep -q '^lxc.cgroup2.devices.allow: c 116:\* rwm$' "$CONF" || \
  echo 'lxc.cgroup2.devices.allow: c 116:* rwm' >> "$CONF"
grep -q '^lxc.mount.entry: /dev/snd dev/snd ' "$CONF" || \
...
echo "Dynamic ALSA passthrough configured. Restart CT $CTID when convenient."
